...
class GPlayer(nn.Module):
    ...
    def __init__(self, gamma2, ell, sigma2):
        super(GPlayer, self).__init__()

        self.gamma2 = nn.Parameter(torch.tensor(gamma2), requires_grad=True).float()
        self.ell = nn.Parameter(torch.tensor(ell), requires_grad=True).float()
        self.sigma2 = nn.Parameter(torch.tensor(sigma2), requires_grad=True).float()

    def forward(self, D, Y):
        '''
        :param D: Distance matrix
        :param Y: Subgoal representation
        :return: Z: Latent subgoal space
        '''
        b,l,c = Y.size()
        Y = Y.cpu().float()
        D = D.cpu().float()
        ...
        X = torch.cholesky_solve(Y, K+torch.exp(self.sigma2)*I)
        Z = K.bmm(X)
        return Z
